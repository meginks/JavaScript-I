...
let findLastCarMakeModel = function() {
   let lastCar = inventory.pop(); 
   return "The last car is a " + lastCar.car_make + " " + lastCar.car_model;
}

console.log(findLastCarMakeModel());

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];
carModels = inventory.map(function(car) {return car.car_model});
...
let carYears = []; 
carYears = inventory.map(function(car) {return car.car_year});
...
let checkCar = function (car_year) {
    return car_year <= 2000;
}

oldCars = carYears.filter(checkCar);
...
let BMWAndAudi =[];
    ...
    for (let i = 0; i <= inventory.length; i++) {
        if (inventory.car_make === "BMW") {
        return inventory[i];
        }
        
        if (inventorys.car_make === "Audi") {
        return inventory[i];   
        }
    }
